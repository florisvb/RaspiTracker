...
static void default_status(RASPIVID_STATE *state)
{
   ...
   state->finished          = 0;
   state->width 			= RC_WIDTH;      // use a multiple of 320 (640, 1280)
   state->height 			= RC_HEIGHT;		// use a multiple of 240 (480, 960)
   state->bitrate 			= 17000000; // This is a decent default bitrate for 1080p
   state->framerate 		= VIDEO_FRAME_RATE_NUM;
   state->immutableInput 	= 1;
   state->graymode 			= GRAYMODE;		// Gray (1) much faster than color (0)
...
static MMAL_COMPONENT_T *create_camera_component(RASPIVID_STATE *state)
{
	...
	//  set up the camera configuration
	{
	   ...
	   cam_config.hdr = { MMAL_PARAMETER_CAMERA_CONFIG, sizeof(cam_config) };
   ...
   if (status)
   {
	...
	mmal_port_parameter_set_uint32(camera->control, MMAL_PARAMETER_SHUTTER_SPEED, RC_SHUTTER_SPEED);
	mmal_port_parameter_set_uint32(camera->control, MMAL_PARAMETER_ISO, RC_ISO);
...
{
	mmal_port_parameter_set_uint32(capture->pState->camera_component->control, property_id, value);
}
...
IplImage * raspiCamCvQueryFrame(RaspiCamCvCapture * capture)
{
	...
	//mmal_port_parameter_set_uint32(&state->camera->control, MMAL_PARAMETER_SHUTTER_SPEED, RC_SHUTTER_SPEED);
	//mmal_port_parameter_set_uint32(&state->camera->control, MMAL_PARAMETER_ISO, RC_ISO);
